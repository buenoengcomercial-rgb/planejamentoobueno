-- Fecha a conclusão do Diário ao Proprietário e restringe as políticas de
-- terceirizados a sessões autenticadas. As permissões de leitura e operação
-- existentes para membros da organização permanecem as mesmas.
...
BEGIN
  ...
  -- A tela já oculta a ação para os demais perfis. Esta verificação no banco
  -- impede a mesma operação por uma chamada direta ou por um cliente desatualizado.
  IF v_new_locked AND NOT v_is_owner THEN
    RAISE EXCEPTION 'Somente o Proprietário pode concluir um Diário.';
  END IF;
...
-- A Equipe de campo pode editar apenas Diários que continuem abertos.
...
-- Estas políticas já exigiam organização e função corretas; agora também são
-- explicitamente inacessíveis a anon/public, eliminando o papel público.
REVOKE ALL ON TABLE public.subcontracts FROM PUBLIC;
REVOKE ALL ON TABLE public.subcontracts FROM anon;
...
DROP POLICY IF EXISTS subcontracts_select_org_members ON public.subcontracts;
CREATE POLICY subcontracts_select_org_members ON public.subcontracts
...
DROP POLICY IF EXISTS subcontracts_insert_owner_admin ON public.subcontracts;
CREATE POLICY subcontracts_insert_owner_admin ON public.subcontracts
...
DROP POLICY IF EXISTS subcontracts_update_owner_admin ON public.subcontracts;
CREATE POLICY subcontracts_update_owner_admin ON public.subcontracts
FOR UPDATE TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.projects p
...
DROP POLICY IF EXISTS subcontracts_delete_owner_admin ON public.subcontracts;
CREATE POLICY subcontracts_delete_owner_admin ON public.subcontracts
