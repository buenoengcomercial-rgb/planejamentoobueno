...
describe('Cronograma do Aditivo', () => {
  it('mantém tarefas virtuais isoladas e sincroniza apenas as datas contratadas', () => {
    ...
    expect(merged.phases[0].tasks.find(task => task.id === 'task-2')?.startDate).toBe('2026-09-01');
  ...
  it('separa valores mensais e preserva supressões negativas', () => {
    const rows = [
      ...
      { taskId: '2', phaseId: 'p', phaseName: 'P', description: 'Suspenso', classification: 'contracted_suspended' as const, statusLabel: '', startDate: '2026-08-10', duration: 5, dependencies: [], responsible: '', quantity: 1, unitPriceWithBDI: 80, totalWithBDI: 80 },
      { taskId: '3', phaseId: 'p', phaseName: 'P', description: 'Supressão', classification: 'proposed_suppression' as const, statusLabel: '', startDate: '2026-08-17', duration: 5, dependencies: [], responsible: '', quantity: -1, unitPriceWithBDI: 50, totalWithBDI: -50 },
    ];
    const forecast = buildAdditiveScheduleForecast(rows);
    expect(forecast).toMatchObject({ totalContractedReleased: 100, totalContractedSuspended: 80, totalProposed: -50 });
  });

  it('gera as três abas do Excel e um PDF paginado', async () => {
    const withDraft = confirmAdditiveScheduleDates(syncAdditiveScheduleDraft(project, additive.id), additive.id);
    const active = withDraft.additives![0];
    const preview = buildAdditiveSchedulePreviewProject(withDraft, active, active.scheduleDraft!);
    const rows = (await import('./additiveSchedule')).buildAdditiveScheduleRows(withDraft, active, preview);
