...
describe('WarehouseBudgetMaterialsTab', () => {
  it('mostra acréscimos e supressões de aditivos nos cabeçalhos ordenáveis', () => {
    const project = {
    ...
    const withdrawnHeader = screen.getByRole('button', { name: 'Retirado líquido' });

    expect(screen.getByRole('button', { name: 'Contratado' })).toBeInTheDocument();
    expect(withdrawnHeader).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Aditivo' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Total planejado' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Suprimido' })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Situação' })).toBeInTheDocument();
    ...
    fireEvent.click(withdrawnHeader);
    expect(withdrawnHeader.closest('th')).toHaveAttribute('aria-sort', 'ascending');
    expect(view.container.querySelector('tbody tr')?.textContent).toContain('Alumínio');
  });
});
