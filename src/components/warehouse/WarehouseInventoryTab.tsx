  ...
  return (
    <div className="grid gap-3 lg:grid-cols-[300px_1fr]">
      <aside className="space-y-3">
        ...
        <div className="overflow-hidden rounded-xl border bg-card"><WarehouseSectionHeader title="Sessões" description={`${sessions.length} registro(s)`} icon={ClipboardCheck} tone="neutral" />{sessions.map(session => <button key={session.id} type="button" className={`w-full border-b p-3 text-left last:border-0 ${selected?.id === session.id ? 'bg-primary/10' : 'hover:bg-muted/30'}`} onClick={() => setSelectedId(session.id)}><div className="flex justify-between gap-2"><strong className="text-sm">{session.number}</strong><span className="text-xs">{session.month}</span></div><div className="mt-2 flex flex-wrap items-center gap-2"><WarehouseStatusBadge label={session.status.replaceAll('_', ' ')} tone={inventoryTone(session.status)} /><span className="text-xs text-muted-foreground">{session.lines.length} material(is)</span></div></button>)}{!sessions.length && <WarehouseEmptyState message="Nenhuma sessão" hint="Abra o inventário do mês." icon={ClipboardCheck} className="m-2" />}</div>
      </aside>
      ...
      <section className="overflow-hidden rounded-xl border bg-card">
        {!selected ? <WarehouseEmptyState message="Nenhum inventário selecionado" hint="Abra o inventário do mês para iniciar." icon={ClipboardCheck} className="m-3 min-h-52" /> : <>
          <div className="flex flex-col gap-3 border-b bg-primary/5 p-3 sm:flex-row sm:flex-wrap sm:items-center"><div><h3 className="text-lg font-bold">{selected.number}</h3><div className="mt-2 flex flex-wrap items-center gap-2"><WarehouseStatusBadge label={selected.status.replaceAll('_', ' ')} tone={inventoryTone(selected.status)} /><span className="text-sm text-muted-foreground">{selected.status === 'em_contagem' ? `${counted} de ${total} materiais contados` : `${total} material(is)`}</span></div></div><div className="grid grid-cols-1 gap-2 sm:ml-auto sm:flex sm:flex-wrap">{selected.status === 'em_contagem' && <><Button variant="outline" className="min-h-11 bg-background" onClick={cancel}><X className="mr-2 h-4 w-4" />Cancelar</Button><Button className="min-h-11 font-bold" onClick={close} disabled={counted !== total || total === 0}><ClipboardCheck className="mr-2 h-4 w-4" />Encerrar contagem</Button></>}{selected.status === 'em_revisao' && <Button className="min-h-11 font-bold" disabled={!canApprove} onClick={apply}><Check className="mr-2 h-4 w-4" />{canApprove ? 'Confirmar e aplicar ajustes' : 'Aguardando administrador'}</Button>}{selected.status === 'aplicado' && <><Button variant="outline" className="min-h-11 bg-background" onClick={() => exportCsv(selected)}><FileDown className="mr-2 h-4 w-4" />CSV</Button><Button variant="outline" className="min-h-11 bg-background" onClick={() => generateInventoryReportPdf(project, selected)}><FileDown className="mr-2 h-4 w-4" />PDF</Button></>}</div></div>
