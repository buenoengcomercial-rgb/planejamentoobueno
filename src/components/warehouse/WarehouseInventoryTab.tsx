...
const inventoryTone = (status: WarehouseInventorySession['status']): WarehouseTone => {
  ...
  if (status === 'em_contagem') return 'info';
...
const normalizeInventorySearch = (value: string) => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('pt-BR').trim();
...
export default function WarehouseInventoryTab({ project, onProjectChange, onCommitWarehouseScoped, auditActor, canApprove = true, canDelete = false }: Props) {
  ...
  const filteredLines = useMemo(() => {
    if (!selected) return [];
    const tokens = normalizeInventorySearch(lineSearch).split(/\s+/).filter(Boolean);
    if (!tokens.length) return selected.lines;
    return selected.lines.filter(line => {
      const haystack = normalizeInventorySearch([line.itemCode, line.itemDescription, line.itemUnit].filter(Boolean).join(' '));
      return tokens.every(token => haystack.includes(token));
    });
  }, [lineSearch, selected]);
  const linePageCount = Math.max(1, Math.ceil(filteredLines.length / INVENTORY_PAGE_SIZE));
  const visibleLines = useMemo(() => filteredLines.slice((linePage - 1) * INVENTORY_PAGE_SIZE, linePage * INVENTORY_PAGE_SIZE), [filteredLines, linePage]);
  useEffect(() => setLinePage(1), [lineSearch, selected?.id]);
  useEffect(() => {
