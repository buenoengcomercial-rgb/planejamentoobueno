...
import { ArrowDown, ArrowUp, ClipboardCheck, PackageSearch, Search, Target } from 'lucide-react';
...
export default function WarehouseBudgetMaterialsTab({ project }: { project: Project }) {
  ...
  const plannedTotal = rows.reduce((total, row) => total + row.totalQuantity, 0);
  const withdrawnTotal = rows.reduce((total, row) => total + row.withdrawnQuantity, 0);
  const additiveRows = rows.filter(row => row.additiveQuantity > 0).length;
