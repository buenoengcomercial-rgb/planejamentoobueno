...
export default function WarehouseBudgetMaterialsTab({ project }: { project: Project }) {
  ...
  return <section className="space-y-3">
    <div className="rounded-xl border bg-card p-3"><h3 className="font-semibold">Materiais do orçamento</h3><p className="mt-1 text-xs text-muted-foreground">Consulta por capítulo principal. Retirado soma somente as requisições entregues vinculadas ao capítulo. Materiais 100% suprimidos permanecem separados pela quantidade total subtraída e não entram no retirado.</p></div>
    ...
    <div className="space-y-2 md:hidden">{rows.map(row => <article key={row.key} className={`rounded-lg border bg-card p-3 ${row.totalQuantity <= 0 ? 'border-destructive/40 bg-destructive/5' : ''}`}><div className="flex items-start justify-between gap-2"><div><p className="font-medium">{row.description}</p><p className="text-xs text-muted-foreground">{row.code ? `${row.bank ? `${row.bank} ` : ''}${row.code} · ` : ''}{row.unit}</p></div>{row.totalQuantity <= 0 ? <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-[10px] font-semibold text-destructive">100% suprimido</span> : row.additiveQuantity > 0 && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-900">Com aditivo</span>}</div><dl className="mt-3 grid grid-cols-2 gap-2 text-xs"><div><dt className="text-muted-foreground">Total planejado</dt><dd className="font-semibold">{qty(row.totalQuantity)}</dd></div><div><dt className="text-muted-foreground">Retirado</dt><dd className="font-semibold text-primary">{qty(row.withdrawnQuantity)}</dd></div><div><dt className="text-muted-foreground">Suprimido</dt><dd className="font-semibold text-destructive">{row.suppressedQuantity ? `-${qty(row.suppressedQuantity)}` : '—'}</dd></div><div><dt className="text-muted-foreground">Aditivo</dt><dd className="font-semibold">{row.additiveQuantity ? qty(row.additiveQuantity) : '—'}</dd></div></dl>{row.additiveStatuses.length > 0 && <p className="mt-2 text-[11px] text-amber-800">Aditivo: {row.additiveStatuses.join(', ')}</p>}</article>)}</div>
    <div className="hidden overflow-x-auto rounded-xl border md:block"><table className="w-full min-w-[930px] text-sm"><thead className="bg-muted/70 text-xs text-muted-foreground"><tr><SortableHeader label="Material" sortKey="description" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Un." sortKey="unit" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Contratado" align="right" sortKey="contractedQuantity" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Aditivo" align="right" sortKey="additiveQuantity" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Total planejado" align="right" sortKey="totalQuantity" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Retirado" align="right" sortKey="withdrawnQuantity" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Suprimido" align="right" sortKey="suppressedQuantity" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /><SortableHeader label="Situação" sortKey="situation" currentKey={sort.key} direction={sort.direction} onSort={toggleSort} /></tr></thead><tbody>{rows.map(row => <tr key={row.key} className={`border-t ${row.totalQuantity <= 0 ? 'bg-destructive/5' : ''}`}><td className="p-3"><div className="font-medium">{row.description}</div>{row.code && <div className="text-xs text-muted-foreground">{row.bank} {row.code}</div>}</td><td className="p-3">{row.unit}</td><td className="p-3 text-right tabular-nums">{qty(row.contractedQuantity)}</td><td className="p-3 text-right tabular-nums">{row.additiveQuantity ? qty(row.additiveQuantity) : '—'}</td><td className="p-3 text-right font-semibold tabular-nums">{qty(row.totalQuantity)}</td><td className="bg-primary/5 p-3 text-right font-semibold tabular-nums text-primary">{qty(row.withdrawnQuantity)}</td><td className="p-3 text-right font-semibold tabular-nums text-destructive">{row.suppressedQuantity ? `-${qty(row.suppressedQuantity)}` : '—'}</td><td className="p-3">{row.totalQuantity <= 0 ? <span className="rounded-full bg-destructive/10 px-2 py-1 text-xs font-semibold text-destructive">100% suprimido</span> : row.additiveQuantity > 0 ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-900">Com aditivo · {row.additiveStatuses.join(', ')}</span> : <span className="text-muted-foreground">Contratado</span>}</td></tr>)}</tbody></table></div>
