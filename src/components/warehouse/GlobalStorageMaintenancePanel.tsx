...
export default function GlobalStorageMaintenancePanel({ currentProject, onCurrentProjectChange, saveProject, organizationId }: Props) {
  ...
  const scan = async () => {
    setAuditing(true);
    try { setReport(await auditStorageMaintenance(organizationId)); }
  ...
  const deleteOrphans = async () => {
    const paths = report?.groups.flatMap(entry => entry.orphans.map(object => object.path)) ?? [];
