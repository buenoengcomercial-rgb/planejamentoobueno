...
interface EquipmentEditForm extends Omit<EquipmentForm, 'confidence'> { name: string; }
...
export default function WarehouseEquipmentsTab({ project, onProjectChange, onCommitWarehouseScoped, auditActor, canArchive = true, canDelete = false, canManageGroups = true, canEdit = true }: Props) {
  ...
  const [equipmentEditForm, setEquipmentEditForm] = useState<EquipmentEditForm>({ name: '', description: '', brand: '', model: '', serial: '', patrimony: '', category: '', notes: '' });
  ...
  const addPhotos = async (files: FileList | null) => {
    ...
    try {
      // Desmonta as prévias existentes antes de decodificar a próxima foto.
      // Em celulares de memória reduzida, manter a primeira miniatura e o
      // bitmap da câmera simultaneamente pode encerrar a página ao adicionar a
      // segunda foto.
      await new Promise<void>(resolve => {
        if (typeof requestAnimationFrame === 'function') requestAnimationFrame(() => resolve());
        else setTimeout(resolve, 0);
      });
      // A Galeria pode devolver mais de um arquivo. Otimizar em sequência evita
      // três decodificações de alta resolução concorrentes.
      const optimized: File[] = [];
      for (const file of accepted) optimized.push(await optimizeEquipmentPhoto(file));
  ...
  const saveEquipmentEdit = async () => {
    ...
    try {
      ...
      const next = updateEquipment(project, editingEquipment.id, {
        ...
        model: equipmentEditForm.model.trim() || undefined,
        serial: equipmentEditForm.serial.trim() || undefined,
  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-xl border bg-card">
        ...
        <div data-testid="equipment-gallery" className="grid grid-cols-1 gap-3 p-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{displayEntries.map(entry => entry.kind === 'group' ? <Fragment key={entry.group.id}><EquipmentGroup group={entry.group} equipments={entry.items} canArchive={canArchive} canDelete={canDelete} canEdit={canEdit} canManage={canManageGroups} expanded={expandedGroupIds.includes(entry.group.id)} onToggle={() => setExpandedGroupIds(current => current.includes(entry.group.id) ? current.filter(id => id !== entry.group.id) : [...current, entry.group.id])} onOpenPhoto={attachment => void openPhoto(attachment)} onPrintLabel={equipment => void printLabel(equipment)} onArchive={equipment => confirm({ title: 'Arquivar equipamento?', description: 'O equipamento e seus termos continuarão no histórico.', confirmLabel: 'Arquivar' }, () => onProjectChange(removeEquipment(project, equipment.id, auditActor)))} onDelete={equipment => void deleteEquipment(equipment)} onEditEquipment={openEditEquipment} onEdit={openEditGroup} onDeleteGroup={removeGroup} />{expandedGroupIds.includes(entry.group.id) && <div data-testid="equipment-group-items" className="contents">{entry.items.map(equipment => <EquipmentCard key={equipment.id} equipment={equipment} canArchive={canArchive} canDelete={canDelete} canEdit={canEdit} onOpenPhoto={attachment => void openPhoto(attachment)} onPrintLabel={equipment => void printLabel(equipment)} onArchive={equipment => confirm({ title: 'Arquivar equipamento?', description: 'O equipamento e seus termos continuarão no histórico.', confirmLabel: 'Arquivar' }, () => onProjectChange(removeEquipment(project, equipment.id, auditActor)))} onDelete={equipment => void deleteEquipment(equipment)} onEdit={openEditEquipment} />)}</div>}</Fragment> : <EquipmentCard key={entry.equipment.id} equipment={entry.equipment} canArchive={canArchive} canDelete={canDelete} canEdit={canEdit} onOpenPhoto={attachment => void openPhoto(attachment)} onPrintLabel={equipment => void printLabel(equipment)} onArchive={equipment => confirm({ title: 'Arquivar equipamento?', description: 'O equipamento e seus termos continuarão no histórico.', confirmLabel: 'Arquivar' }, () => onProjectChange(removeEquipment(project, equipment.id, auditActor)))} onDelete={equipment => void deleteEquipment(equipment)} onEdit={openEditEquipment} selectable={groupMode && canManageGroups} selected={selectedEquipmentIds.includes(entry.equipment.id)} onToggleSelection={toggleEquipmentSelection} />)}{!equipments.length && <div className="col-span-full"><WarehouseEmptyState message="Nenhum equipamento cadastrado" hint="Use Adicionar equipamento para começar." icon={HardHat} /></div>}</div>
      ...
      <Dialog open={!!editingEquipment} onOpenChange={open => { if (!open) setEditingEquipment(null); }}>
        <DialogContent className="warehouse-ui flex max-h-[95dvh] w-[calc(100vw-1rem)] max-w-3xl flex-col gap-0 overflow-hidden p-0 [&>button]:h-11 [&>button]:w-11">
          <DialogHeader className="border-b p-4 pr-16"><DialogTitle>Editar cadastro do equipamento</DialogTitle><DialogDescription>Código interno, fotos, status e cautelas permanecem preservados. Atualize os dados de identificação abaixo.</DialogDescription></DialogHeader>
          <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4"><div className="grid gap-3 sm:grid-cols-2"><WarehouseField label="Nome de referência"><Input aria-label="Nome de referência" className="min-h-11 text-base" value={equipmentEditForm.name} onChange={event => setEquipmentEditForm(current => ({ ...current, name: event.target.value }))} /></WarehouseField><WarehouseField label="Descrição"><Input aria-label="Descrição" className="min-h-11 text-base" value={equipmentEditForm.description} onChange={event => setEquipmentEditForm(current => ({ ...current, description: event.target.value }))} /></WarehouseField><WarehouseField label="Marca" optional><Input aria-label="Marca" className="min-h-11 text-base" value={equipmentEditForm.brand} onChange={event => setEquipmentEditForm(current => ({ ...current, brand: event.target.value }))} /></WarehouseField><WarehouseField label="Modelo" optional><Input aria-label="Modelo" className="min-h-11 text-base" value={equipmentEditForm.model} onChange={event => setEquipmentEditForm(current => ({ ...current, model: event.target.value }))} /></WarehouseField><WarehouseField label="Nº de série" optional><Input aria-label="Nº de série" className="min-h-11 text-base" value={equipmentEditForm.serial} onChange={event => setEquipmentEditForm(current => ({ ...current, serial: event.target.value }))} /></WarehouseField><WarehouseField label="Patrimônio existente" optional><Input aria-label="Patrimônio existente" className="min-h-11 text-base" value={equipmentEditForm.patrimony} onChange={event => setEquipmentEditForm(current => ({ ...current, patrimony: event.target.value }))} /></WarehouseField><WarehouseField label="Categoria" optional><Input aria-label="Categoria" className="min-h-11 text-base" value={equipmentEditForm.category} onChange={event => setEquipmentEditForm(current => ({ ...current, category: event.target.value }))} /></WarehouseField><div className="sm:col-span-2"><WarehouseField label="Observações" optional><Input aria-label="Observações" className="min-h-11 text-base" value={equipmentEditForm.notes} onChange={event => setEquipmentEditForm(current => ({ ...current, notes: event.target.value }))} /></WarehouseField></div></div></div>
          <DialogFooter className="gap-2 border-t bg-background p-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] sm:space-x-0"><Button variant="outline" className="min-h-11 sm:min-w-28" onClick={() => setEditingEquipment(null)}>Cancelar</Button><Button className="min-h-11 sm:min-w-44" disabled={!equipmentEditForm.description.trim()} onClick={saveEquipmentEdit}>Salvar alterações</Button></DialogFooter>
        </DialogContent>
      </Dialog>
...
function EquipmentCard({ equipment, canArchive, canDelete, canEdit, onOpenPhoto, onPrintLabel, onArchive, onDelete, onEdit, selectable = false, selected = false, onToggleSelection }: EquipmentCardProps) {
  ...
  return <article data-testid="equipment-card" className={`min-w-0 overflow-hidden rounded-xl border bg-card shadow-sm transition-shadow hover:shadow-md ${selected ? 'border-primary ring-2 ring-primary/25' : ''}`}>
    ...
    <div className="space-y-2 p-3">
      ...
      {!selectable && <div className={`grid gap-1.5 ${canArchive || canEdit || canDelete ? 'grid-cols-2' : 'grid-cols-1'}`}>
        ...
        {canEdit && <Button variant="outline" className="min-h-11 px-2 text-xs" onClick={() => onEdit(equipment)}><Pencil className="mr-1.5 h-4 w-4" />Editar</Button>}
...
function EquipmentPhoto({ file, suspended = false, onRemove }: { file: File; suspended?: boolean; onRemove: () => void }) {
  const url = useMemo(() => URL.createObjectURL(file), [file]);
  useEffect(() => () => URL.revokeObjectURL(url), [url]);
  return <div className="relative aspect-square overflow-hidden rounded-md border">{suspended ? <span className="flex h-full w-full items-center justify-center bg-muted text-xs text-muted-foreground">Preparando…</span> : <img src={url} alt={file.name} className="h-full w-full object-cover" />}<Button size="icon" variant="destructive" className="absolute right-1 top-1 h-8 w-8" onClick={onRemove} aria-label={`Remover ${file.name}`}><X className="h-4 w-4" /></Button></div>;
}
