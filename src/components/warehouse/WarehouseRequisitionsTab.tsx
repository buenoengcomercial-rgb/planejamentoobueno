  ...
  return (
    <div className="space-y-3">
      ...
      <section className="overflow-hidden rounded-xl border bg-card">
          ...
          <div className="space-y-4 p-2 md:hidden">{buildingGroups.map(building => <section key={building.key} data-testid="withdrawal-building-group" className="space-y-3 rounded-lg border border-primary/25 bg-primary/10 p-2"><HistoryGroupHeader title={building.label} requisitionCount={building.requisitions.length} itemCount={building.itemCount} />{building.dateGroups.map(dateGroup => <section key={dateGroup.key} data-testid="withdrawal-date-group" className={`overflow-hidden rounded-lg border shadow-sm ${isDateGroupExpanded(dateGroup) ? 'border-primary/50 bg-primary/15' : 'border-border bg-muted/55'}`}><WithdrawalDateGroupHeader dateGroup={dateGroup} expanded={isDateGroupExpanded(dateGroup)} onToggle={() => toggleDateGroup(dateGroup)} onGenerate={() => generateDailyConfirmations(building, dateGroup)} />{isDateGroupExpanded(dateGroup) && <div className="ml-3 space-y-3 border-l-4 border-t border-primary/60 border-t-primary/35 bg-primary/10 p-2">{dateGroup.requisitions.map((requisition, index) => <WithdrawalHistoryCard key={requisition.id} project={project} requisition={requisition} movements={wh.movements} alternate={index % 2 === 1} active={expandedRequisitionIds.has(requisition.id)} canDelete={canDelete} canEdit={canEdit} onToggle={() => setExpandedRequisitionIds(current => { const next = new Set(current); if (next.has(requisition.id)) next.delete(requisition.id); else next.add(requisition.id); return next; })} onDelete={() => deleteRequisition(requisition)} onReturn={() => setReturnTarget(requisition)} onCorrect={() => setCorrectionTarget(requisition)} />)}</div>}</section>)}</section>)}{!wh.requisitions.length && <WarehouseEmptyState message="Nenhuma retirada registrada" hint="Use Nova retirada para começar." />}</div>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1040px] text-xs"><thead className="bg-muted"><tr><th className="w-10 p-2"><span className="sr-only">Detalhes</span></th><th className="p-2 text-left">Nº</th><th className="p-2 text-left">Data da operação</th><th className="p-2 text-left">Último registro</th><th className="p-2 text-left">Recebedor</th><th className="p-2 text-left">Hierarquia / destino</th><th className="p-2 text-center">Itens</th><th className="p-2 text-left">Status</th><th className="p-2 text-left">Incluído / alterado por</th></tr></thead><tbody>{buildingGroups.map(building => <Fragment key={building.key}><tr data-testid="withdrawal-building-group" className="border-y border-primary/30 bg-primary/15"><td colSpan={9} className="px-3 py-2"><HistoryGroupHeader title={building.label} requisitionCount={building.requisitions.length} itemCount={building.itemCount} /></td></tr>{building.dateGroups.map(dateGroup => <Fragment key={dateGroup.key}><tr data-testid="withdrawal-date-group" className={`border-y ${isDateGroupExpanded(dateGroup) ? 'border-primary/50 bg-primary/20' : 'border-border bg-muted/80'}`}><td colSpan={9} className="px-3 py-2"><WithdrawalDateGroupHeader dateGroup={dateGroup} expanded={isDateGroupExpanded(dateGroup)} onToggle={() => toggleDateGroup(dateGroup)} onGenerate={() => generateDailyConfirmations(building, dateGroup)} /></td></tr>{isDateGroupExpanded(dateGroup) && dateGroup.requisitions.map((requisition, index) => <WithdrawalHistoryRow key={requisition.id} project={project} requisition={requisition} movements={wh.movements} alternate={index % 2 === 1} active={expandedRequisitionIds.has(requisition.id)} canDelete={canDelete} canEdit={canEdit} onToggle={() => setExpandedRequisitionIds(current => { const next = new Set(current); if (next.has(requisition.id)) next.delete(requisition.id); else next.add(requisition.id); return next; })} onDelete={() => deleteRequisition(requisition)} onReturn={() => setReturnTarget(requisition)} onCorrect={() => setCorrectionTarget(requisition)} />)}</Fragment>)}</Fragment>)}{!wh.requisitions.length && <tr><td colSpan={9} className="p-8 text-center text-muted-foreground">Nenhuma retirada registrada.</td></tr>}</tbody></table></div>
      </section>
...
function WithdrawalHistoryCard({ project, requisition, movements, alternate, active, canDelete, canEdit, onToggle, onDelete, onReturn, onCorrect }: WithdrawalHistoryEntryProps) {
  ...
  return <article className={`overflow-hidden rounded-md border ${active ? 'border-primary/70 bg-primary/25 shadow-sm' : alternate ? 'border-primary/35 bg-primary/15' : 'border-primary/35 bg-primary/10'}`}><button type="button" className={`w-full p-3 text-left transition-colors hover:bg-primary/25 ${active ? 'bg-primary/30 hover:bg-primary/35' : ''}`} onClick={onToggle} aria-expanded={active}><div className="flex justify-between gap-2"><strong>{requisition.number}</strong><ChevronDown className={`h-4 w-4 transition-transform ${active ? 'rotate-180 text-primary' : ''}`} /></div><div className="mt-1 text-sm">{requisition.receiverName || requisition.requesterName || '—'}</div><div className="text-xs text-muted-foreground">{chapter} · {requisition.items.length} item(ns)</div><div className="mt-1 text-xs text-muted-foreground">Operação: {formatOperationalDate(requisition.date)} · Último registro: {formatRecordedAt({ createdAt: latest }, requisition.date)}</div></button>{active && <div data-testid="withdrawal-history-details" className="ml-6 mr-2 border-l-4 border-primary bg-primary/15 p-3"><WithdrawalDetails project={project} requisition={requisition} canDelete={canDelete} canEdit={canEdit} onDelete={onDelete} onReturn={onReturn} onCorrect={onCorrect} /></div>}</article>;
}

function WithdrawalHistoryRow({ project, requisition, movements, alternate, active, canDelete, canEdit, onToggle, onDelete, onReturn, onCorrect }: WithdrawalHistoryEntryProps) {
  const chapter = chapterPathLabel(project, requisition.chapterId, requisition.chapterName || requisition.taskName);
  const latest = latestRequisitionActivity(requisition, movements);
  return <Fragment><tr data-testid="withdrawal-history-row" className={`cursor-pointer border-t border-primary/30 whitespace-nowrap transition-colors hover:bg-primary/25 ${active ? 'border-primary/70 bg-primary/30 hover:bg-primary/35' : alternate ? 'bg-primary/15' : 'bg-primary/10'}`} onClick={onToggle}><td className="border-l-4 border-l-primary bg-primary/20 py-2 pl-10 pr-2"><ChevronDown className={`h-4 w-4 transition-transform ${active ? 'rotate-180 text-primary' : ''}`} /></td><td className="p-2 font-mono">{requisition.number}</td><td className="p-2">{formatOperationalDate(requisition.date)}</td><td className="p-2">{formatRecordedAt({ createdAt: latest }, requisition.date)}</td><td className="p-2">{requisition.receiverName || requisition.requesterName || '—'}</td><td className="max-w-80 truncate p-2" title={chapter}>{chapter}</td><td className="p-2 text-center">{requisition.items.length}</td><td className="p-2"><WarehouseStatusBadge label={requisition.status === 'rascunho' ? 'Pendente legado' : 'Entregue'} tone={requisition.status === 'rascunho' ? 'warning' : 'success'} /></td><td className="p-2"><WarehouseAuditIdentity createdBy={requisition.createdBy} updatedBy={requisition.updatedBy} createdAt={requisition.createdAt} updatedAt={requisition.updatedAt} className="space-y-0.5" /></td></tr>{active && <tr data-testid="withdrawal-history-details" className="bg-primary/15"><td colSpan={9} className="py-3 pl-12 pr-4"><div className="rounded-lg border border-primary/45 border-l-4 border-l-primary bg-primary/20 p-3"><WithdrawalDetails project={project} requisition={requisition} canDelete={canDelete} canEdit={canEdit} onDelete={onDelete} onReturn={onReturn} onCorrect={onCorrect} /></div></td></tr>}</Fragment>;
}
