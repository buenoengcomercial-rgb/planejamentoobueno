...
function WithdrawalHistoryCard({ project, requisition, movements, alternate, active, canDelete, canEdit, onToggle, onDelete, onReturn, onCorrect }: WithdrawalHistoryEntryProps) {
  ...
  return <article className={`overflow-hidden rounded-md border ${active ? 'border-success/70 bg-success/15 shadow-sm' : alternate ? 'border-border bg-muted/45' : 'border-border bg-background'}`}><button type="button" className={`w-full p-3 text-left transition-colors hover:bg-muted/70 ${active ? 'bg-success/20 hover:bg-success/25' : ''}`} onClick={onToggle} aria-expanded={active}><div className="flex justify-between gap-2"><strong>{requisition.number}</strong><ChevronDown className={`h-4 w-4 transition-transform ${active ? 'rotate-180 text-success' : ''}`} /></div><div className="mt-1 text-sm">{requisition.receiverName || requisition.requesterName || '—'}</div><div className="text-xs text-muted-foreground">{chapter} · {requisition.items.length} item(ns)</div><div className="mt-1 text-xs text-muted-foreground">Operação: {formatOperationalDate(requisition.date)} · Último registro: {formatRecordedAt({ createdAt: latest }, requisition.date)}</div></button>{active && <div data-testid="withdrawal-history-details" className="ml-4 mr-2 border-l-4 border-success/70 bg-success/5 p-3"><WithdrawalDetails project={project} requisition={requisition} canDelete={canDelete} canEdit={canEdit} onDelete={onDelete} onReturn={onReturn} onCorrect={onCorrect} /></div>}</article>;
}
...
function WithdrawalHistoryRow({ project, requisition, movements, alternate, active, canDelete, canEdit, onToggle, onDelete, onReturn, onCorrect }: WithdrawalHistoryEntryProps) {
  const chapter = chapterPathLabel(project, requisition.chapterId, requisition.chapterName || requisition.taskName);
  const latest = latestRequisitionActivity(requisition, movements);
  return <Fragment><tr data-testid="withdrawal-history-row" className={`cursor-pointer border-t border-border whitespace-nowrap transition-colors hover:bg-muted/65 ${active ? 'border-success/70 bg-success/20 hover:bg-success/25' : alternate ? 'bg-muted/40' : 'bg-background'}`} onClick={onToggle}><td className="border-l-4 border-l-primary/35 bg-primary/5 py-2 pl-10 pr-2"><ChevronDown className={`h-4 w-4 transition-transform ${active ? 'rotate-180 text-success' : ''}`} /></td><td className="p-2 font-mono">{requisition.number}</td><td className="p-2">{formatOperationalDate(requisition.date)}</td><td className="p-2">{formatRecordedAt({ createdAt: latest }, requisition.date)}</td><td className="p-2">{requisition.receiverName || requisition.requesterName || '—'}</td><td className="max-w-80 truncate p-2" title={chapter}>{chapter}</td><td className="p-2 text-center">{requisition.items.length}</td><td className="p-2"><WarehouseStatusBadge label={requisition.status === 'rascunho' ? 'Pendente legado' : 'Entregue'} tone={requisition.status === 'rascunho' ? 'warning' : 'success'} /></td><td className="p-2"><WarehouseAuditIdentity createdBy={requisition.createdBy} updatedBy={requisition.updatedBy} createdAt={requisition.createdAt} updatedAt={requisition.updatedAt} className="space-y-0.5" /></td></tr>{active && <tr data-testid="withdrawal-history-details" className="bg-success/5"><td colSpan={9} className="py-3 pl-8 pr-4"><div className="rounded-lg border border-success/30 border-l-4 border-l-success/70 bg-success/5 p-3"><WithdrawalDetails project={project} requisition={requisition} canDelete={canDelete} canEdit={canEdit} onDelete={onDelete} onReturn={onReturn} onCorrect={onCorrect} /></div></td></tr>}</Fragment>;
}
