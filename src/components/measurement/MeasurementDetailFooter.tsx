...
function scopeValues(row: Row, scope: MeasurementValueScope | undefined) {
  if (scope === 'period') return { label: 'Subtotal da medição', withBdi: row.valuePeriod, noBdi: row.valuePeriodNoBDI };
...
function AnalyticView({ composition }: { composition?: AdditiveComposition }) {
  if (!composition?.inputs?.length) {
    return <p className="text-sm text-muted-foreground">Esta linha ainda não possui composição analítica vinculada.</p>;
  }
...
function QuantityView({ row }: { row: Row }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
      ...
      <div className="rounded-md border border-border bg-background px-3 py-2">
        <div className="text-[11px] text-muted-foreground">Qtd. medição</div>
      ...
      <p className="sm:col-span-4 text-xs text-muted-foreground">
        Na Medição, a memória é leitura: as quantidades vêm dos apontamentos da EAP/Tarefas/Diário de Obra.
      </p>
    </div>
  );
}
...
function ClassificationView({
  ...
  const breakdown = composition ? MC.getMaterialCompositionBreakdown(project, composition, 'Medição') : null;
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-2">
        ...
        <div className="rounded-md border border-primary/30 bg-primary/5 px-3 py-2">
          ...
          <div className="text-[10px] text-muted-foreground">{bdi.toLocaleString('pt-BR')}% no orçamento</div>
        </div>
      </div>
      ...
      {!breakdown && (
        <p className="text-xs text-muted-foreground">
          Não encontrei analítica vinculada para separar este valor por Material, Mão de obra e Equipamento.
        </p>
      )}
    </div>
  );
...
export default function MeasurementDetailFooter({ project, selection, row, bdi }: Props) {
  ...
  const title =
    selection?.mode === 'quantity' ? 'Memória / quantidades da medição'
      ...
      <div className="px-4 py-3 border-b border-border bg-muted/30">
        ...
        {row && (
          <p className="mt-0.5 text-xs text-muted-foreground truncate">
            {row.item || 'Sem item'} · {row.itemCode || 'sem código'} · {row.description}
          </p>
        )}
      </div>
      <div className="p-3 max-h-[320px] overflow-auto">
        {!selection || !row ? (
          <div className="text-sm text-muted-foreground">
            Clique em uma quantidade, preço ou valor total para ver os detalhes neste rodapé.
          </div>
