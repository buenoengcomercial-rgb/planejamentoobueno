        ...
        {step === 'preview' && (
          <div className="space-y-4 py-2">
            ...
            {kind === 'synthetic' ? <p className="rounded border p-4 text-sm">O arquivo <strong>{file?.name}</strong> criará o aditivo <strong>{name}</strong> em rascunho, com <strong>{syntheticAdditive?.compositions.length ?? 0}</strong> composições.</p> : preview && <>
              ...
              <div className="max-h-80 overflow-auto rounded border"><table className="w-full text-xs"><thead className="sticky top-0 bg-muted"><tr><th className="p-2 text-left">Excel</th><th className="p-2 text-left">Destino</th><th className="p-2 text-right">Insumos</th><th className="p-2 text-right">Referência</th><th className="p-2 text-right">Após desconto</th><th className="p-2 text-right">Com BDI</th><th className="p-2 text-right">Diverg.</th><th className="p-2">Situação</th></tr></thead><tbody>{preview.matches.map(match => { const row = computeAdditiveRow({ ...(active!.compositions.find(c => c.id === match.targetCompositionId) ?? active!.compositions[0]), isNewService: true, analyticReferenceUnitPriceNoBDI: match.referenceUnitPriceNoBDI }, bdi, globalDiscount); return <tr key={match.blockIndex} className="border-t"><td className="p-2"><strong>{match.code}</strong><div className="text-muted-foreground">{match.bank}</div></td><td className="p-2">{match.targetItem ?? '—'}</td><td className="p-2 text-right">{match.inputCount}</td><td className="p-2 text-right">{money(match.referenceUnitPriceNoBDI)}</td><td className="p-2 text-right">{money(row.unitPriceNoBDIWithDiscount)}</td><td className="p-2 text-right">{money(row.unitPriceWithBDI)}</td><td className="p-2 text-right">{match.priceDivergences}</td><td className="p-2 text-center"><Badge variant="outline" className={match.status === 'matched' ? 'text-emerald-700' : match.status === 'conflict' ? 'text-rose-700' : 'text-amber-700'}>{match.status === 'matched' ? 'Vinculada' : match.status === 'conflict' ? 'Conflito' : 'Sem vínculo'}</Badge>{match.reason && <span title={match.reason}><AlertTriangle className="inline ml-1 h-3 w-3" /></span>}</td></tr>; })}</tbody></table></div>
            </>}
          </div>
        )}
